...
LightSensor lightSensorRight(A3, 2.31, -700);
// 定义PID结构体变量
...
void raytracing() {
    ...
    if (debug) {
        ...
        if (++cnt >= 50) {
            cnt = 0;
            char str[30];
            sprintf(str, "errorX: %d errorY: %d\n", (int)(errorX),
                    (int)(errorY));
            Serial.print(str);
            sprintf(str, "Up: %ld Down: %ld\nLeft: %ld Right: %ld\n",
    ...
    switch (cmd) {
        case 'H':  // Horizontal
            ...
            Serial.println("Horizontal: " +
                           String(horizontalControl ? "on" : "off"));
            break;
        case 'V':  // Vertical
            verticalControl = !verticalControl;
            Serial.println("Vertical: " +
                           String(verticalControl ? "on" : "off"));
            break;
        case '!':  // Debug
            debug = !debug;
        ...
        case 'X':  // pidX
            ...
            Serial.println("pidX: kp=" + String(pidX.kp, 6) + ", ki=" +
        ...
        case 'Y':  // pidX
            ...
            Serial.println("pidY: kp=" + String(pidY.kp, 6) + ", ki=" +
...
void printParams() {
    ...
    Serial.println("Right: k=" + String(lightSensorRight.k) +
                   ", b=" + String(lightSensorRight.b));
    // 输出当前PID的参数
    Serial.println("pidX: kp=" + String(pidX.kp, 6) +
                   ", ki=" + String(pidX.ki, 6) + ", kd=" + String(pidX.kd, 6));
    Serial.println("pidY: kp=" + String(pidY.kp, 6) +
                   ", ki=" + String(pidY.ki, 6) + ", kd=" + String(pidY.kd, 6));
    // 输出当前阈值
    Serial.println("Threshold: " + String(lightThreshold));
}